...
with open(fileName, 'rb') as file:
    ...
    # clean the firings but trimming to leave only the contracting interval
    ...
    ###### Plot the final MUAPs ##########

    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    axes = axes.flatten()
    nplot = 0

    for i in range(nmus):  # Loop over 8 cells
        ax = axes[nplot]  # Get the subplot axes
        for j in range(chans2use):  # Loop over 16 channels
